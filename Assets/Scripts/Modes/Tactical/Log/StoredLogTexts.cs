...
static class StoredLogTexts
{
    ...
    internal static void InitializeLists()
    {
        ...
        SwallowMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> taunts \"Ta-da! No hands!\" However, <b>{i.Target.Name}</b> {GetRandomStringFrom("is not amused by the trick.", "is unimpressed.", $"remains displeased at {GPPHis(i.Target)} current situation.")}",
