...
static class StoredLogTexts
{
    ...
    internal static void InitializeLists()
    {
        ...
        GreatEscapeKeepMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> stretches, feeling the pleasant heaviness of <b>{i.Target.Name}</b> in {GPPHis(i.Unit)} {GetRandomStringFrom("lower torso", "pouch", "marsupium")}.",priority:25, conditional: HasGreatEscape && InPouch(s)),
            ...
            new EventString((i) => $"<b>{i.Target.Name}</b> kicks the {GetRandomStringFrom("lower torso", "pouch", "marsupium")} holding {GPPHim(i.Target)} from the inside. <b>{i.Unit.Name}</b> considers getting rid of {GPPHis(i.Unit)} rude occupant right now.",priority:25, conditional: HasGreatEscape && InPouch(s)),
