...
static class StoredLogTexts
{
    ...
    internal static void InitializeLists()
    {
        ...
        SwallowMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> {GetRandomStringFrom("hisses", "mewls", "meows")} at the {GetRaceDescSingl(i.Target)} taunting {GPPHim(i.Target)} in the feline native language{GetRandomStringFrom(".", $". <b>{i.Target.Name}</b> not recognizing the dialect simply wonders if the {GetRaceDescSingl(i.Unit)} has gone feral.", $". <b>{i.Target.Name}</b>, recognizing some words begins to bark back in the canine native language!", $". <b>{i.Target.Name}</b>, recognizing some words begins to bark back in the canine native language. Soon <b>{i.Unit.Name}</b> is in a full blown argument with {GPPHis(i.Unit)} own {i.preyLocation.ToSyn()}!")}",
            ...
            actorRace: Race.Xelhilde, targetRace: Race.Cats, priority: 10),
            new EventString((i) => $"<b>{i.Unit.Name}</b> {GetRandomStringFrom("swallows", "gulps down", "vores", "devours")} <b>{i.Target.Name}</b> {GetRandomStringFrom("angrily", "furiously", "indignantly")} and barks, \"Funny how I can't use my paws to wield my weapon yet you can, and you <i>still</i> lost!\"{GetRandomStringFrom("", $" <b>{i.Target.Name}</b> now begins to stew in TWO ways after such a {GetRandomStringFrom("scathing", "harsh", "brutal", "salty")} remark from a fellow dog.")}",
            actorRace: Race.Xelhilde, targetRace: Race.Dogs, priority: 9, conditional: s => !s.Target.IsDead && s.Target.Side != s.Unit.Side),
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> {GetRandomStringFrom("laments", "whimpers", "whines")} \"{GetRandomStringFrom("I am sorry it has to be like this.", "We should have been allies.", "Why must we be enemies?")}\"",
            actorRace: Race.Xelhilde, targetRace: Race.Dogs, priority: 9, conditional: s => !s.Target.IsDead && s.Target.Side != s.Unit.Side),
            ...
            actorRace: Race.Xelhilde, priority: 8, conditional: s => TargetHumanoid(s) && !s.Target.IsDead),
