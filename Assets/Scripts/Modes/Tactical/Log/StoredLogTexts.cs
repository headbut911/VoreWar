...
static class StoredLogTexts
{
    ...
    internal static void InitializeLists()
    {
        ...
        bool InBreasts(EventLog s) => (s.preyLocation == PreyLocation.breasts && s.Unit.Race != Race.Kangaroos) || s.preyLocation == PreyLocation.leftBreast || s.preyLocation == PreyLocation.rightBreast;
        bool InBalls(EventLog s) => s.preyLocation == PreyLocation.balls;
        bool InPouch(EventLog s) => s.preyLocation == PreyLocation.breasts && s.Unit.Race == Race.Kangaroos;
        bool InTail(EventLog s) => s.preyLocation == PreyLocation.tail;
        ...
        SwallowMessages = new List<EventString>()
        {
            ...
            actorRace: Race.Cats, targetRace: Race.Cats, priority: 10),
            //Kitties vs. Doggos
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> {GetRandomStringFrom("barks", "snarls", "growls")} at the {GetRaceDescSingl(i.Target)} taunting {GPPHim(i.Target)} in the canine native language{GetRandomStringFrom(".", $". <b>{i.Target.Name}</b> not recognizing the dialect simply wonders if the {GetRaceDescSingl(i.Unit)} has gone feral.", $". <b>{i.Target.Name}</b>, recognizing some words begins to hiss back in the feline native language!", $". <b>{i.Target.Name}</b>, recognizing some words begins to hiss back in the feline native language. Soon <b>{i.Unit.Name}</b> is in a full blown argument with {GPPHis(i.Unit)} own {i.preyLocation.ToSyn()}!")}",
            actorRace: Race.Dogs, targetRace: Race.Cats, priority: 9),
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> {GetRandomStringFrom("nyans", "mewls", "meows")} at the {GetRaceDescSingl(i.Target)} taunting {GPPHim(i.Target)} in the feline native language{GetRandomStringFrom(".", $". <b>{i.Target.Name}</b> not recognizing the dialect simply wonders if the {GetRaceDescSingl(i.Unit)} has gone feral.", $". <b>{i.Target.Name}</b>, recognizing some words begins to bark back in the canine native language!", $". <b>{i.Target.Name}</b>, recognizing some words begins to bark back in the canine native language. Soon <b>{i.Unit.Name}</b> is in a full blown argument with {GPPHis(i.Unit)} own {i.preyLocation.ToSyn()}!")}",
            actorRace: Race.Cats, targetRace: Race.Dogs, priority: 9),
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> {GetRandomStringFrom("barks", "snarls", "growls")} \"You're right where you belong! You {GetRandomStringFrom("snarky", "prissy", "tasty", "delicious")} cat!\" as {GPPHe(i.Unit)} {GetRandomStringFrom("pat", "rub", "pet", "massage")}{SIfSingular(i.Unit)} {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}!",
            actorRace: Race.Dogs, targetRace: Race.Cats, priority: 10),
            new EventString((i) => $"After {GetRandomStringFrom("nimbly", "agilely")} {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> {GetRandomStringFrom("mewls", "purrs", "meows")} mockingly \"{GetRandomStringFrom("Stupid", "Foolish", "Dumb")} {GetRandomStringFrom("mutt", "dog", "hound")} you should know by now {GetRandomStringFrom("who the superior species is", "not to pick a fight with a cat")}!\" as {GPPHe(i.Unit)} {GetRandomStringFrom("pat", "rub", "knead", "massage")}{SIfSingular(i.Unit)} {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}!",
            actorRace: Race.Cats, targetRace: Race.Dogs, priority: 10),
            //Xelhilde
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> can't help but {GetRandomStringFrom("bark", "snarl", "growl")} \"Why do stupid cats get to use their grubby little paws to use weapons but I'm stuck on all fours using my mouth to wield my sword!\"{GetRandomStringFrom("", $" <b>{i.Target.Name}</b> realizing the semi-feral {ApostrophizeWithOrWithoutS(GetRaceDescSingl(i.Unit))} disadvantage, just feels <i>more</i> upset at the fact that they are stuck in her {i.preyLocation.ToSyn()}!")}",
            actorRace: Race.Xelhilde, targetRace: Race.Cats, priority: 10),
            new EventString((i) => $"<b>{i.Unit.Name}</b> {GetRandomStringFrom("swallows", "gulps down", "vores", "devours")} <b>{i.Target.Name}</b> and gloats, \"How do you feel having been eaten by a warrior that can't even use their paws to wield their weapon, stupid cat?\"{GetRandomStringFrom("", $" Judging by the hissing that soon emanates from her {i.preyLocation.ToSyn()}; Not very happy.")}",
            actorRace: Race.Xelhilde, targetRace: Race.Cats, priority: 10),
            new EventString((i) => $"<b>{i.Unit.Name}</b> {GetRandomStringFrom("swallows", "gulps down", "vores", "devours")} <b>{i.Target.Name}</b> {GetRandomStringFrom("angrily", "furiously", "indignantly")} and {GetRandomStringFrom("barks", "says")} \"Funny how I can't use my paws to wield my weapon yet you can, and you <i>still</i> lost!\" <b>{i.Target.Name}</b> now begins to stew in TWO ways after such a {GetRandomStringFrom("scathing", "harsh", "brutal", "salty")} remark from a fellow dog.",
            actorRace: Race.Xelhilde, targetRace: Race.Dogs, priority: 9, conditional: s => !s.Target.IsDead && s.Target.Side != s.Unit.Side),
            new EventString((i) => $"<b>{i.Unit.Name}</b> {GetRandomStringFrom("swallows", "gulps down", "vores", "devours")} <b>{i.Target.Name}</b> and says \"You've been a bad {GetGenderString(i.Target, "girl", "boy", "dog")}! I hope you enjoy your stay in there because I know I will!\" as she resumes panting adorably.",
            actorRace: Race.Xelhilde, targetRace: Race.Dogs, priority: 9, conditional: s => !s.Target.IsDead && s.Target.Side != s.Unit.Side),
            new EventString((i) => $"<b>{i.Unit.Name}</b> {GetRandomStringFrom("swallows", "gulps down", "vores", "devours")} <b>{i.Target.Name}</b> and says \"You've been a bad {GetGenderString(i.Target, "girl", "boy", "dog")}! Now stay in there and thick about what you've done!\" as she resumes panting adorably.",
            actorRace: Race.Xelhilde, targetRace: Race.Dogs, priority: 9, conditional: s => !s.Target.IsDead && s.Target.Side != s.Unit.Side),
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> complements the {GetRandomStringFrom("mighty", "fearsome", "strong", "fierce")} {GetRaceDescSingl(i.Target)}, \"{GetRandomStringFrom("Well fought!", "If you fall here, just know you were a honorable opponent.", "Your fighting spirit is strong!")}\"{GetRandomStringFrom("", $"In response <b>{i.Target.Name}</b> feels somewhat honored!", $"In response <b>{i.Target.Name}</b> feels honored and reinvigorated to continue to make {GPPHis(i.Unit)} escape!", $"In response <b>{i.Target.Name}</b> suddenly feels a warrior’s bond with <b>{i.Unit.Name}</b>!")}",
            actorRace: Race.Xelhilde, priority: 10, conditional: s => ((s.Target.Race == Race.Tigers) || (s.Target.Race == Race.Panthers)) && !s.Target.IsDead && s.Target.Side != s.Unit.Side),
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> taunts \"Ta-da! No hands!\" <b>{i.Target.Name}</b> {GetRandomStringFrom("is not amused by the trick.", "is unimpressed.", $"remains displeased at {GPPHis(i.Target)} current situation.")}",
            actorRace: Race.Xelhilde, priority: 9, conditional: s => TargetHumanoid(s) && !s.Target.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> {GetRandomStringFrom("swallows", "gulps down", "vores", "devours")} <b>{i.Target.Name}</b> and {GetRandomStringFrom("tauntingly", "mockingly", "indignantly")} {GetRandomStringFrom("barks", "says")} \"Funny how I can't use my paws to wield my weapon yet you can, and you <i>still</i> lost! Well, I hope you enjoy your stay in there because I know I will!\" <b>{i.Target.Name}</b> now begins to stew in TWO ways after such a {GetRandomStringFrom("scathing", "harsh", "brutal", "salty")} remark from a semi-feral {GetRandomStringFrom("hound", "canine", "doberman", "dog")}.",
            actorRace: Race.Xelhilde, priority: 9, conditional: s => TargetHumanoid(s) && !s.Target.IsDead && s.Target.Side != s.Unit.Side && s.Target.HasWeapon && s.Target.FixedGear == false),
            ...
            actorRace: Race.Crux, targetRace: Race.Crux, priority: 10),
            //Aabayx exclusive creds to Tatltuae!
            ...
            priority: 8),
            new EventString((i) => $"<b>{i.Unit.Name}</b> grabs <b>{i.Target.Name}</b>, shoving the {GetRaceDescSingl(i.Target)} down {GPPHis(i.Unit)} {GetRandomStringFrom("hot", "humid", "moist", "slick", "sloppy", "wet", "slimy", "dripping", "sopping", "drooling", "steamy", "eager")} {GetRandomStringFrom("gullet", "throat", "esophagus")}!",
            priority: 8),
            new EventString((i) => {
            ...
            //FeralEevee Swallow. Creds to Tatltuae for FeralEevee lines!
            new EventString((i) => $"As <b>{i.Unit.Name}</b> plays with <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> accidently slips {GPPHis(i.Unit)} playmate into {GPPHis(i.Unit)} mouth and reflexivally swallows {GetRandomStringFrom($"{GPPHim(i.Target)}", $"the {GetRaceDescSingl(i.Target)}")} down {GPPHis(i.Unit)} throat.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s)),
            new EventString((i) => $"After seeing <b>{i.Target.Name}</b> make aggresive motions, <b>{i.Unit.Name}</b> decides that the {GetRaceDescSingl(i.Target)} must be punished, and swiftly swallows {GPPHim(i.Target)}.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> playfully leaps at <b>{i.Target.Name}</b>, mouth open. A few moments later, the confused Eevee with an enlarged stomach wonders where {GPPHis(i.Unit)} new friend went.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s)),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> plays with <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> swallows <b>{i.Target.Name}</b> down for safe keeping.",
            actorRace: Race.FeralEevee, priority: 12, conditional: s => TargetHumanoid(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> decides that the {GetRaceDescSingl(i.Target)} must be rewarded, and swiftly swallows {GPPHim(i.Target)} into {GPPHis(i.Unit)} comfy belly.",
            actorRace: Race.FeralEevee, priority: 12, conditional: s => TargetHumanoid(s) && Endo(s)),
        ...
        RandomDigestionMessages = new List<EventString>()
        {
            ...
            targetRace: Race.Slimes, priority: 9, conditional: (s) => InPouch(s) == false),
            ...
            priority: 8, conditional: (s) => InPouch(s) == false),
            ...
            targetRace: Race.Deer, priority: 9, conditional: (s) => InPouch(s) == false),
            ...
            priority: 8, conditional: (s) => FirstTime(s) && InPouch(s) == false),
            ...
            actorRace: Race.FeralOrcas, priority: 9, conditional: s => Scat(s)),
            ...
            targetRace: Race.FeralOrcas, priority: 9, conditional: (s) => InPouch(s) == false),
            ...
            priority: 11, actorRace: Race.Feit),
            new EventString((i) => $"<b>{i.Unit.Name}'s</b> bowels eagerly growl for <b>{i.Target.Name}</b>... ready to {GetRandomStringFrom("reduce them to", "churn them into", "melt them into", "make them into")} {GetRaceDescSingl(i.Unit)} {PreyLocStrings.ScatSyn()}!",
            ...
            priority: 11, targetRace: Race.Feit, conditional: (s) => InPouch(s) == false),
            ...
            priority: 9, conditional: (s) => FirstTime(s) && InPouch(s) == false),
            ...
            priority: 9, conditional: (s) => FirstTime(s) && ActorHumanoid(s) && InPouch(s) == false),
            ...
            priority: 9, conditional: (s) => FirstTime(s) && ActorHumanoid(s) && TargetHumanoid(s) && InPouch(s) == false),
            ...
            priority: 9, conditional: (s) => FirstTime(s) && HardVore(s) && InPouch(s) == false),
            ...
            priority: 9, conditional: s => Cursed(s) && InPouch(s) == false),
            ...
            priority: 11, conditional: s => Friendly(s) && Endo(s) && InPouch(s) == false),
            ...
            priority: 12, conditional: s => Friendly(s) && Endo(s) && Cursed(s) && InPouch(s) == false),
            ...
            priority: 11, conditional: s => Friendly(s) && HealingEndo(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: s => Friendly(s) && !Endo(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: s => Friendly(s) && !Endo(s) && InPouch(s) == false && s.Unit.HasWeapon),
            ...
            priority: 8, conditional: (s) => Friendly(s) && HardVore(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: s => Friendly(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: (s) => s.Target.HasTrait(Traits.Eternal) && InPouch(s) == false),
            ...
            priority: 8, conditional: (s) => ReqOSWLewd(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: (s) =>ReqSSWAndOSW(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: s =>ReqOSWLewd(s) && HardVore(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: s => ReqOSW(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: s => Lewd(s) && HardVore(s) && InPouch(s) == false),
            ...
            priority: 9, conditional: s=> Cursed(s) && Lewd(s) && InPouch(s) == false),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Humans, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Taurus, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Taurus, priority: 9, conditional: s => InPouch(s) && (s.Target.HasSpecificWeapon(ItemType.CompoundBow) || s.Target.HasSpecificWeapon(ItemType.Bow))),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Taurus, priority: 9, conditional: s => InPouch(s) && (s.Target.HasSpecificWeapon(ItemType.Axe) || s.Target.HasSpecificWeapon(ItemType.Mace))),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Deer, priority: 9, conditional: s => InPouch(s) && !s.Target.HasDick),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Kangaroos, priority: 9, conditional: s => InPouch(s) && s.Target.Side != s.Unit.Side),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Dogs, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Cats, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Cats, priority: 9, conditional: InPouch),
            new EventString((i) => $"As a soft rumbling begins to come from <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> pouch, {GetRandomStringFrom($"<b>{i.Unit.Name}</b>", $"the {GetRaceDescSingl(i.Unit)}")} looks at {GPPHis(i.Unit)} pouch for a moment before asking \"Are you purring?\" After another few moments, {GetRandomStringFrom($"<b>{i.Unit.Name}</b>", $"the {GetRaceDescSingl(i.Unit)}")} asks \"Did you fall asleep!?\"",
            actorRace: Race.Kangaroos, targetRace: Race.Panthers, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Foxes, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Bunnies, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Bats, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Ants, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Wolves, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Lizards, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Slimes, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Scylla, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Harpies, priority: 9, conditional: InPouch),
            new EventString((i) => $"<b>{i.Unit.Name}</b> looks down at {GPPHis(i.Unit)} pouch and says to <b>{i.Target.Name}</b> \"Hey, when you feel yourself running out of air, let me know so I can nab a few feathers off of you. I need them for an art project I'm working on.\"",
            actorRace: Race.Kangaroos, targetRace: Race.Avians, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Imps, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Crypters, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Lamia, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Crux, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Equines, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Sergal, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Bees, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Driders, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Alraune, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Merfolk, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Frogs, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Sharks, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Aabayx, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, targetRace: Race.Mice, priority: 9, conditional: InPouch),
            ...
            actorRace: Race.Kangaroos, priority: 9, conditional: s => InPouch(s) && s.Unit.Level * 2 <= s.Target.Level && s.Unit.Level >= 4),
            ...
            actorRace: Race.Kangaroos, priority: 9, conditional: s => InPouch(s) && s.Target.Level * 2 <= s.Unit.Level && s.Unit.Level >= 4),
            ...
            actorRace: Race.Kangaroos, priority: 9, conditional: s => InPouch(s) && s.Unit.HasVagina),
            ...
            actorRace: Race.Kangaroos, priority: 9, conditional: s => InPouch(s) && s.Unit.HasDick),
            ...
            actorRace: Race.Kangaroos, priority: 9, conditional: s => InPouch(s) && TargetHumanoid(s)),
            ...
            actorRace: Race.Kangaroos, priority: 9, conditional: InPouch),
            ...
            //FeralEevee RandomDigests
            new EventString((i) => $"<b>{i.Unit.Name}</b> lets out a small, cute, heart-melting burp. It's also a sign of <b>{i.Target.Name}</b> melting.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InStomach(s) && CanBurp(s)),
            new EventString((i) => $"As {GPPHis(i.Unit)} stomach gurgles ominously, <b>{i.Unit.Name}</b> briefly wonders where <b>{i.Target.Name}</b> went before the predatory Eevee is distracted by something else.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InStomach(s)),
            new EventString((i) => $"As <b>{i.Target.Name}</b> struggles inside <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {GetRandomStringFrom("stomach", "belly")}, {GPPHe(i.Target)} accidentally punch the Eevee's chin. With this, <b>{i.Unit.Name}</b> decides that <b>{i.Target.Name}</b> must be punished.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InStomach(s)),
            new EventString((i) => $"Within <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> womb, <b>{i.Target.Name}</b> tries to reason with {GetRandomStringFrom($"{GPPHis(i.Target)} predator", "the Eevee")}. On the outside, <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> left ear twitches. <b>{i.Target.Name}</b> has been ignored.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InWomb(s)),
            new EventString((i) => $"Inside <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> womb, fluids have begun to rise around <b>{i.Target.Name}</b>. Outside, the Eevee looks for more things to play with.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InWomb(s)),
        ...
        BellyRubMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"As <b>{i.Unit.Name}</b> touches <b>{i.Target.Name}</b>, {GPPHe(i.Unit)} places {GPPHis(i.Unit)} hands atop {GPPHis(i.Target)} {GetRandomStringFrom("sloshing", "sloshy", "gurgling", "gurgly", "churning", "groaning", "growling", "rumbling", "rumbly", "hungry", "hungering", "ravenous", "voracious", "gluttonous", "wailing", "quaking", "quivering", "shifting", "trembling", "wobbling", "stuffed", "full", "bloated", "bulging", "thrashing", "bubbling", "bubbly", "pulsating", "curvaceous", "prey-filled", "prey-stuffed", "soft")} {PreyLocStrings.ToSyn(PreyLocation.stomach)} and starts exploring {GPPHis(i.Target)} body's most sensitive areas, eventually sliding southward {GetRandomStringFrom("to the <i>really</i> sensitive areas", "to the fun zone", "between their sexy thighs", "for some <i>real</i> fun", "to properly get them off")}.",
            priority: 8, conditional: s => s.Target.Side == s.Unit.Side && ReqTargetCompatibleLewd(s)),
            ...
            targetRace: Race.Wolves, priority: 9, conditional: s => (s.Target != s.Unit) && ActorHumanoid(s)),
            new EventString((i) => $"\"Who's a good {GetGenderString(i.Target, "girl", "boy", "wolf")}?\" <b>{i.Unit.Name}</b> says as {GPPHe(i.Unit)} rub{SIfSingular(i.Unit)} <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {GetRandomStringFrom("sloshing", "sloshy", "gurgling", "gurgly", "churning", "groaning", "growling", "rumbling", "rumbly", "hungry", "voracious", "gluttonous", "wailing", "quaking", "quivering", "shifting", "trembling", "wobbling", "stuffed", "full", "bloated", "bulging", "thrashing", "bubbling", "bubbly", "pulsating", "curvaceous", "prey-filled", "prey-stuffed", "soft", "swollen", "lumpy", "deadly", "hefty", "overworked")} {PreyLocStrings.ToSyn(PreyLocation.stomach)}. The wolf lies down, {GPPHis(i.Target)} tongue lolling out, tail rapidly wagging, and leg kicking as <b>{i.Unit.Name}</b> rubs {GPPHis(i.Target)} belly in all the right places. \"I am!\" <b>{i.Target.Name}</b> exclaims.",
            targetRace: Race.Wolves, priority: 9, conditional: s => (s.Target != s.Unit) && ActorHumanoid(s)),
            //Angy Wolfie Rub
            new EventString((i) => $"\"Who's a good {GetGenderString(i.Target, "girl", "boy", "wolf")}?\" <b>{i.Unit.Name}</b> says as {GPPHe(i.Unit)} rub{SIfSingular(i.Unit)} <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {GetRandomStringFrom("sloshing", "sloshy", "gurgling", "gurgly", "churning", "groaning", "growling", "rumbling", "rumbly", "hungry", "voracious", "gluttonous", "wailing", "quaking", "quivering", "shifting", "trembling", "wobbling", "stuffed", "full", "bloated", "bulging", "thrashing", "bubbling", "bubbly", "pulsating", "curvaceous", "prey-filled", "prey-stuffed", "soft", "swollen", "lumpy", "deadly", "hefty", "overworked")} {PreyLocStrings.ToSyn(PreyLocation.stomach)}. The wolf lies down, {GPPHis(i.Target)} tongue lolling out, tail rapidly wagging, and leg kicking as <b>{i.Unit.Name}</b> rubs {GPPHis(i.Target)} belly in all the right places.",
            targetRace: Race.FeralWolves, priority: 9, conditional: s => (s.Target != s.Unit) && ActorHumanoid(s)),
            ...
            targetRace: Race.Dogs, priority: 9, conditional: s => (s.Target != s.Unit) && ActorHumanoid(s)),
            new EventString((i) => $"\"Who's a good {GetGenderString(i.Target, "girl", "boy", "dog")}?\" <b>{i.Unit.Name}</b> says as {GPPHe(i.Unit)} rub{SIfSingular(i.Unit)} <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {GetRandomStringFrom("sloshing", "sloshy", "gurgling", "gurgly", "churning", "groaning", "growling", "rumbling", "rumbly", "hungry", "voracious", "gluttonous", "wailing", "quaking", "quivering", "shifting", "trembling", "wobbling", "stuffed", "full", "bloated", "bulging", "thrashing", "bubbling", "bubbly", "pulsating", "curvaceous", "prey-filled", "prey-stuffed", "soft", "swollen", "lumpy", "deadly", "hefty", "overworked")} {PreyLocStrings.ToSyn(PreyLocation.stomach)}. The dog lies down, {GPPHis(i.Target)} tongue lolling out, tail rapidly wagging, and leg kicking as <b>{i.Unit.Name}</b> rubs {GPPHis(i.Target)} belly in all the right places. \"I am! I am a good {GetGenderString(i.Target, "girl", "boy", "dog")}!\" <b>{i.Target.Name}</b> happily exclaims.",
            targetRace: Race.Dogs, priority: 9, conditional: s => (s.Target != s.Unit) && ActorHumanoid(s)),
            ...
            targetRace: Race.RwuMercenaries, priority: 9, conditional: s => InStomach(s)),
            ...
            targetRace: Race.RwuMercenaries, actorRace: Race.RwuMercenaries, priority: 9, conditional: s => InStomach(s)),
            ...
            priority: 11, conditional: s => s.Target == s.Unit && InStomach(s), actorRace: Race.RwuMercenaries),
            new EventString((i) => {
            ...
            priority: 11, conditional: s => s.Target == s.Unit && SizeDiff(s, 1) && InStomach(s), actorRace: Race.RwuMercenaries),
            //FeralEevee Rubs
            //Fellow Eevee rubs
            new EventString((i) => $"<b>{i.Unit.Name}</b> nuzzles <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> middle, and then they play together a bit, further melting <b>{i.Prey.Name}</b>.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> nuzzles <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> middle, and then they play together a bit, stirring up the liquid that once was <b>{i.Prey.Name}</b>.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> cuddles up to <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> engorged belly, unwittingly taking a few minutes off of <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> life.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> and <b>{i.Unit.Name}</b> take a minute to play-fight, the movements causing more digestive fluid to flood <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> current \"home.\"",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> accidentally runs face first into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly, unknowingly giving <b>{i.Prey.Name}</b> a powerful headbutt.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> cuddles up to <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> engorged belly, causing the belly to shrink as more of <b>{i.Prey.Name}</b> is absorbed by <b>{i.Target.Name}</b>.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> and <b>{i.Unit.Name}</b> take a minute to play-fight, sloshing around the liquid remains of <b>{i.Prey.Name}</b> within <b>{i.Target.Name}</b>.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> accidentally runs face first into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly, {GPPHis(i.Unit)} head works to stir up the remains of <b>{i.Prey.Name}</b>, further converting {GPPHim(i.Prey)} to liquid.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Target.Name}</b> and <b>{i.Unit.Name}</b> take a minute to play-fight, <b>{i.Target.Name}</b> feels a warm, pleasent sensation in {GPPHis(i.Target)} womb. Not knowing what it is, but liking it, <b>{i.Target.Name}</b> plays harder, making <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> life just that much more miserable.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> rubs {GPPHis(i.Unit)} face into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> middle, shortening <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> remaining time before {GPPHe(i.Prey)} become{SIfSingular(i.Prey)} {(i.Target.HasTrait(Traits.PredGusher) ? "a white splater" : "a new Eevee.")}",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s) && !s.Prey.IsDead && Config.KuroTenkoEnabled && (Config.UBConversion == UBConversion.Both || Config.UBConversion == UBConversion.RebirthOnly)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> rubs {GPPHis(i.Unit)} face into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> middle, shortening <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> remaining time before {GPPHe(i.Prey)} become{SIfSingular(i.Prey)} {(i.Target.HasTrait(Traits.PredGusher) ? "a white splater" : "a new friend.")}",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s) && !s.Prey.IsDead && Config.KuroTenkoEnabled && (Config.UBConversion == UBConversion.Both || Config.UBConversion == UBConversion.ConversionOnly)),
            new EventString((i) => $"As <b>{i.Target.Name}</b> and <b>{i.Unit.Name}</b> take a minute to play-fight, <b>{i.Target.Name}</b> feels a warm, pleasent sensation in {GPPHis(i.Target)} womb. Not knowing what it is, but liking it, <b>{i.Target.Name}</b> plays harder, further mixing <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> remains.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s) && s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> rubs {GPPHis(i.Unit)} face into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> middle, dissolving <b>{i.Prey.Name}</b> into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> womb just a little more.",
            targetRace: Race.FeralEevee, actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s) && s.Prey.IsDead),
            //Rubs from Non-Eevees
            new EventString((i) => $"<b>{i.Unit.Name}</b> cautiously approaches <b>{i.Target.Name}</b>, and begins petting the Eevee's belly. Within, the stomach acids rise a little more around <b>{i.Prey.Name}</b>.",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InStomach(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> approaches <b>{i.Unit.Name}</b>, and stares at {GPPHim(i.Unit)} intently. Reading the small creature's wants, <b>{i.Unit.Name}</b> and <b>{i.Target.Name}</b> play for a bit, helping the Eevee to digest <b>{i.Prey.Name}</b>.",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InStomach(s) && !s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> presses {GPPHis(i.Unit)} hands into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly, {GPPHe(i.Unit)} whisper{SIfSingular(i.Unit)} to <b>{i.Prey.Name}</b>. \"How does it feel to be digested by a creature that has already forgotten you exist?\"",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InStomach(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> cautiously approaches <b>{i.Target.Name}</b>, and begins petting the Eevee's belly. The movements drive a little more of <b>{i.Prey.Name}</b> into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> intestines, shrinking <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly a little.",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> presses {GPPHis(i.Unit)} hands into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly, {GPPHe(i.Unit)} whisper{SIfSingular(i.Unit)} to <b>{i.Prey.Name}</b>. \"I know you're dead now, but how does it feel to be digested by a creature smaller than you?\"",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> cautiously approaches <b>{i.Target.Name}</b>, and begins petting the Eevee's middle. Seeing a hand sticking out of <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}, <b>{i.Unit.Name}</b> casually pushes it back in.",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InWomb(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> approaches <b>{i.Unit.Name}</b>, and stares at {GPPHim(i.Unit)} intently. Reading the small creature's wants, <b>{i.Unit.Name}</b> and <b>{i.Target.Name}</b> play for a bit, getting the Eevee somewhat worked up, and helping {GPPHis(i.Target)} womb with the task of killing off <b>{i.Prey.Name}</b>.",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InWomb(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> cautiously approaches <b>{i.Target.Name}</b>, and begins petting the Eevee's middle. Seeing some clear-ish fluid leak from the Eevee's {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}, <b>{i.Unit.Name}</b> snirks, knowing what has happened to <b>{i.Prey.Name}</b>{GetRandomStringFrom($", and knowing that <b>{i.Target.Name}</b> doesn't know.", ".")}",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InWomb(s) && s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> cautiously approaches <b>{i.Target.Name}</b>, and begins petting the Eevee's middle. Seeing some clear-ish fluid leak from the Eevee's {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}, <b>{i.Unit.Name}</b> snirks, knowing what has happened to <b>{i.Prey.Name}</b>. \"You look good as Eevee cum, hope to see more of you soon.\"",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InWomb(s) && s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> approaches <b>{i.Unit.Name}</b>, and stares at {GPPHim(i.Unit)} intently. Reading the small creature's wants, <b>{i.Unit.Name}</b> and <b>{i.Target.Name}</b> play for a bit, getting the Eevee somewhat worked up. <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> womb quivers a little, absorbing more of <b>{i.Prey.Name}</b>.",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InWomb(s) && s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> presses {GPPHis(i.Unit)} hands into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly, {GPPHe(i.Unit)} whisper{SIfSingular(i.Unit)} to <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> remains tauntingly. \"Better hope <b>{i.Target.Name}</b> is feeling like being a mommy, or else you're just Eevee cum in the making at this point.\"",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && InWomb(s) && s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> presses {GPPHis(i.Unit)} hands into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly, {GPPHe(i.Unit)} whisper{SIfSingular(i.Unit)} to <b>{i.Prey.Name}</b>. \"Will you come out as cum, yourself but with an altered mind, or an Eevee? I'm personally rooting for the {GetRandomStringFrom("first option, I never liked you", "second option, I like the way you look", "third option, I think you would look cute as an Eevee")}.\"",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && s.Target.Side != s.Unit.Side && !s.Prey.IsDead && InWomb(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> approaches <b>{i.Unit.Name}</b>, and stares at {GPPHim(i.Unit)} intently. Reading the small creature's wants, <b>{i.Unit.Name}</b> and <b>{i.Target.Name}</b> play for a bit, turning more of <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> corpse to liquid.",
            targetRace: Race.FeralEevee, priority: 11, conditional: s => ActorHumanoid(s) && HardVore(s) && InStomach(s) && s.Prey.IsDead),
            //Eevees can rub themselves, don't ya know!
            new EventString((i) => $"<b>{i.Target.Name}</b> decides to roll all the way over {GPPHis(i.Target)} enlarged {GetRandomStringFrom("belly", "middle")}, like a strange wheel. Absolutly no part of this process is at all comfortable for <b>{i.Prey.Name}</b>.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && !s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Target.Name}</b> starts trying to remember where <b>{i.Prey.Name}</b> went, for reasons beyond the Eevee's comprehention, {GPPHis(i.Target)} {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")} gets warm, and whenever those weird wiggling sensations come from {GPPHis(i.Target)} womb, the Eevee's {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")} grows even warmer somehow.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InWomb(s) && !s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Target.Name}</b> starts trying to remember where <b>{i.Prey.Name}</b> went, for reasons beyond the Eevee's comprehention, {GPPHis(i.Target)} {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")} gets warm, and whenever those weird wiggling sensations come from {GPPHis(i.Target)} womb(\"what are those anyways?\") <b>{i.Target.Name}</b> wonders, the Eevee's {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")} grows even warmer somehow.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InWomb(s) && !s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Target.Name}</b> starts thinking about <b>{RandomAlliedWarrior(i.Target).Name}</b>, feeling an unfamiliar feeling(which we might call being \"horny\"), the Eevee's {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")} grows warm, aiding <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> progress towards the \"most embaressing death\" award.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InWomb(s) && !s.Prey.IsDead && ReqRAW(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> decides to roll all the way over {GPPHis(i.Target)} enlarged {GetRandomStringFrom("belly", "middle")}, like a strange wheel. Each rotation, <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {GetRandomStringFrom("belly", "middle")} shrinks down further.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InWomb(s) && s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Target.Name}</b> starts trying to remember where <b>{i.Prey.Name}</b> went, for reasons beyond the Eevee's comprehention, {GPPHis(i.Target)} {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")} gets warm, and a weird fluid has begun leaking from there as well.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InWomb(s) && s.Prey.IsDead),
            new EventString((i) => $"As <b>{i.Target.Name}</b> starts thinking about <b>{RandomAlliedWarrior(i.Target).Name}</b>, feeling an unfamiliar feeling(which we might call being \"horny\"), the Eevee's {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")} grows warm, speeding <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> absorbtion.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InWomb(s) && s.Prey.IsDead && ReqRAW(s)),
            new EventString((i) => $"As <b>{i.Prey.Name}</b> tries to stretch out, the image of {GPPHis(i.Prey)} face morphing out from the front of {GPPHis(i.Target)} belly frightens <b>{i.Target.Name}</b>, who whacks <b>{i.Prey.Name}</b> straight in the face with one of {GPPHis(i.Target)} front paws.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InStomach(s) && !s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> decides to bounce up and down on {GPPHis(i.Target)} belly, pumping more digestive fluids in and forcing more of <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> limited air out as cute little Eevee burps.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InStomach(s) && !s.Prey.IsDead && CanBurp(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> curls up on {GPPHis(i.Target)} engorged belly, {GPPHis(i.Target)} weight pushing a little more of <b>{ApostrophizeWithOrWithoutS(i.Prey.Name)}</b> liquid form down into the Eevee's intestines.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> decides to bounce up and down on {GPPHis(i.Target)} belly, pumping more of the liquified <b>{i.Prey.Name}</b> down into {GPPHis(i.Target)} intestines.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => s.Target == s.Unit && InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"<b>{i.Target.Name}</b> curls up on {GPPHis(i.Target)} engorged belly, {GPPHis(i.Target)} weight rubbing more acids into <b>{i.Prey.Name}</b>.",
        ...
        {
            new EventString((i) =>$"<b>{i.Unit.Name}</b> massages {(i.Unit == i.Target ? GPPHis(i.Target) : "<b>" + i.Target.Name + "</b>'s")} full breasts.", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> massages {(i.Unit == i.Target ? GPPHis(i.Target) : "<b>" + i.Target.Name + "</b>'s")} full breasts, milk leaking out of {(i.Unit == i.Target ? GPPHis(i.Target) : "<b>" + i.Target.Name + "</b>'s")} engorged nipples.", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> grabs {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}, lifting them up and then letting them bounce against {GPPHis(i.Unit)} chest, sloshing their contents about!", priority: 8, conditional: s => s.Target == s.Unit && InPouch(s) == false),
            new EventString((i) =>$"With a happy sigh, <b>{i.Unit.Name}</b> places {GPPHis(i.Unit)} arms behind {GPPHis(i.Unit)} back and shakes {GPPHis(i.Unit)} torso vigorusly, giving {GPPHis(i.Unit)} {i.preyLocation.ToSyn()} a good stirring. ", priority: 8, conditional: s => s.Target == s.Unit && InPouch(s) == false),
            ...
            actorRace: Race.Kangaroos, priority: 11, conditional: s => InPouch(s) && s.Target != s.Unit && !PreyDead(s)),
            // Other Ver. B
        ...
        BallMassageMessages = new List<EventString>()
        {
            ...
            // New lines (Thanks Tatltuae! :thumbsup:)
        ...
        {
            new EventString((i) =>$"<b>{i.Target.Name}</b> lets out a distressed yelp as {GPPHeIs(i.Target)} shoved headfirst into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> nipple!", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> casts aside {GPPHis(i.Unit)} weapon and grabs onto <b>{i.Target.Name}</b>, stuffing {GPPHim(i.Target)} into {GPPHis(i.Unit)} cleavage!", priority: 8, conditional: (s) => s.Unit.HasWeapon && s.Unit.FixedGear == false && ActorHumanoid(s) && InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> swiftly jumps on <b>{i.Target.Name}</b>, {GPPHis(i.Target)} form engulfed by <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> breasts!", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> shoves <b>{i.Target.Name}</b> onto the ground and shoves {GPPHim(i.Target)} legfirst into {GPPHis(i.Unit)} nipple!", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> overpowers <b>{i.Target.Name}</b> and swiftly shoves {GPPHim(i.Target)} into {GPPHis(i.Unit)} bust!", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> shoves <b>{i.Target.Name}</b> into {GPPHis(i.Unit)} supple, hungry bosom.", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> nipples ache after stretching far enough to consume {GPPHis(i.Unit)} rival warrior.", priority: 8, conditional: (s) => InPouch(s) == false),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> holds <b>{i.Target.Name}</b> tightly, preparing to swallow them, but just as {GPPHe(i.Unit)} go{EsIfSingular(i.Unit)} to swallow them, {GPPHe(i.Unit)} see{SIfSingular(i.Unit)} {GPPHis(i.Unit)} prey disappearing into {GPPHis(i.Unit)} engorged cleavage.", priority: 8, conditional: (s) => InPouch(s) == false),
            ...
            //Slime prey
            new EventString((i) => $"<b>{i.Unit.Name}</b> reaches for <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> core, shoving it into {GPPHis(i.Unit)} cleavage!", targetRace: Race.Slimes, priority: 10, conditional: (s) => InPouch(s) == false),
            new EventString((i) => $"<b>{i.Unit.Name}</b> grabs <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> viscous form and slurps {GPPHim(i.Target)} into {GPPHis(i.Unit)} nipples!", targetRace: Race.Slimes, priority: 10, conditional: (s) => InPouch(s) == false),

            //Kangaroo pred (pouch vore)
            new EventString((i) => $"<b>{i.Unit.Name}</b> grabs <b>{i.Target.Name}</b> and shoves {GPPHim(i.Target)} into {GPPHis(i.Unit)} pouch. Once <b>{i.Target.Name}</b> is fully inside, the pouch entrance seals tight.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            // Todo: Write a method for checking the unit's feet when I'm not so tired
            //new EventString((i) => $"<b>{i.Unit.Name}</b> grabs <b>{i.Target.Name}</b> with both arms and lifts {GPPHim(i.Target)} up. At first, <b>{i.Target.Name}</b> is worried, but as <b>{i.Unit.Name}</b> starts to lower {GPPHim(i.Target)}, <b>{i.Target.Name}</b> feels relieved. That is, until <b>{i.Target.Name}</b> feels {GPPHis(i.Target)} {(i.Target.Race == Race.Lamia ? "tail" : i.Target.Race == Race.Slimes ? "feet")} enter a warm chamber. Before <b>{i.Target.Name}</b> knows it, {GPPHe(i.Target)} find{SIfSingular(i.Target)} the opening of <b>{i.Unit.Name}</b>'s pouch sealing tight above {GPPHim(i.Target)}!", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> approaches <b>{i.Target.Name}</b> and taps on {GPPHis(i.Target)} shoulder. <b>{i.Unit.Name}</b> then gestures to {GPPHis(i.Unit)} pouch, which {GPPHeIs(i.Unit)} holding open with {GPPHis(i.Unit)} fingers. After <b>{i.Target.Name}</b> reluctantly gets in, <b>{i.Unit.Name}</b>'s pouch's entrance closes somewhat above <b>{i.Target.Name}</b>, not enough to block oxygen from flowing in, but enough to stop <b>{i.Target.Name}</b> from just falling out.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s) && (s.Target.Side == s.Unit.Side)),

            // Endo pouch vore
            new EventString((i) => $"<b>{i.Unit.Name}</b> approaches <b>{i.Target.Name}</b> and asks {GPPHim(i.Target)} to get in {GPPHis(i.Unit)} pouch, which <b>{i.Target.Name}</b> quickly does.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s) && (s.Target.Side == s.Unit.Side)),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> walks by <b>{i.Target.Name}</b>, <b>{i.Target.Name}</b> asks if {GPPHe(i.Target)} could hide in <b>{i.Unit.Name}</b>'s pouch for a bit, to which <b>{i.Unit.Name}</b> shrugs and says \"sure.\"", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s) && (s.Target.Side == s.Unit.Side)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> approaches <b>{i.Target.Name}</b> and tells {GPPHim(i.Target)} to get in {GPPHis(i.Unit)} pouch. Not one to question orders, <b>{i.Target.Name}</b> gets in <b>{i.Unit.Name}</b>'s pouch.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s) && (s.Target.Side == s.Unit.Side)),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> approaches <b>{i.Target.Name}</b>, {GPPHe(i.Target)} surprises <b>{i.Unit.Name}</b> by jumping in {GPPHis(i.Unit)} pouch!", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s) && (s.Target.Side == s.Unit.Side)),
        ...
        AnalVoreMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"After a quick playful wrestle, <b>{i.Unit.Name}</b> sits proudly atop <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> head. Then {GPPHe(i.Target)} slip{SIfSingular(i.Target)}, and in a matter of moments, only <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> feet remains in the light, sticking out of an Eevee's anus.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && !(s.Target.Race == Race.Lamia) && !(s.Target.Race == Race.Vipers)),
            new EventString((i) => $"<b>{i.Target.Name}</b> ran the last few moments over in {GPPHis(i.Target)} head. \"I tried to charge the Eevee that was looking the wrong way. I tripped. I fell towards the Eevee's butt. That can only mean...\" shrieks of horror ring from <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> anus, to which <b>{i.Unit.Name}</b>, still facing the wrong direction, is blissfully unaware.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> runs in and around <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {GetRandomStringFrom("legs", "base")}, frolicking happily, and tripping <b>{i.Target.Name}</b> in the process... and somehow falling straight into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {GetRandomStringFrom("anus", "ass", "asshole", "tailhole", "butt")}. As <b>{i.Unit.Name}</b> unknowingly slurps <b>{i.Unit.Name}</b> up, {GPPHe(i.Unit)} simply wonder{SIfSingular(i.Unit)} why {GPPHis(i.Unit)} belly is suddenly so big.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s)),
            new EventString((i) => $"After a quick playful wrestle, <b>{i.Unit.Name}</b> pulls <b>{i.Target.Name}</b> up {GPPHis(i.Unit)} anus to be {GPPHis(i.Unit)} reward until <b>{i.Unit.Name}</b> wants to play another game with <b>{i.Target.Name}</b>.",
            actorRace: Race.FeralEevee, priority: 13, conditional: s => TargetHumanoid(s) && Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> pulls <b>{i.Target.Name}</b> up into {GPPHis(i.Unit)} rectum, or as <b>{i.Unit.Name}</b> likes to think of it, \"friend storage.\"",
            actorRace: Race.FeralEevee, priority: 13, conditional: s => TargetHumanoid(s) && Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> crawls up <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> anus, trying to go unnoticed. <b>{i.Unit.Name}</b>, of course, notices this, but being a good sport, pretends not to notice.",
            actorRace: Race.FeralEevee, priority: 13, conditional: s => TargetHumanoid(s) && Friendly(s) && Endo(s)),
        };
        ...
        UnbirthMessages = new List<EventString>()
        {
            ...
            //FeralEevee UB
            new EventString((i) => $"<b>{i.Unit.Name}</b> jumps on <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> head, intending to play with the {GetRaceDescSingl(i.Target)}, only to find <b>{i.Target.Name}</b> slipping up {GPPHis(i.Unit)} {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}!",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> decides to punish <b>{i.Target.Name}</b> by shoving {GPPHim(i.Target)} up {GPPHis(i.Unit)} {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}. Why? Because <b>{i.Unit.Name}</b> thinks the womb is meant to be the time out corner.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> bowls over <b>{i.Target.Name}</b>, leaving {GPPHim(i.Target)} on {GPPHis(i.Target)} back. Then <b>{i.Unit.Name}</b> sees <b>{PotentialNextPrey(i.Unit).Name}</b> and gets scared and walks backwards, accidently slurping <b>{i.Target.Name}</b> up {GPPHis(i.Unit)} {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && PotentialNextPrey(s.Unit).Name != "You, the player" ),
            new EventString((i) => $"<b>{i.Unit.Name}</b> and <b>{i.Target.Name}</b> decide to play a game. <b>{i.Target.Name}</b> crawls up into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> womb, and tries to stay there while <b>{i.Unit.Name}</b> tries to push {GPPHim(i.Target)} out.",
            actorRace: Race.FeralEevee, priority: 13, conditional: s => TargetHumanoid(s) && Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> and <b>{i.Target.Name}</b> decide to play a game. <b>{i.Target.Name}</b> crawls up into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> womb, and tries to get out while <b>{i.Unit.Name}</b> tries to pull {GPPHim(i.Target)} in.",
            actorRace: Race.FeralEevee, priority: 13, conditional: s => TargetHumanoid(s) && Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> and <b>{i.Target.Name}</b> decide to play a game. <b>{i.Target.Name}</b> goes into the tube between the vagina and womb, and tries to see how long {GPPHe(i.Target)} can stay there without falling into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> womb or back out the vagina.",
            actorRace: Race.FeralEevee, priority: 13, conditional: s => TargetHumanoid(s) && Friendly(s) && Endo(s)),
        };
        ...
        DigestionDeathMessages = new List<EventString>()
        {
            ...
            priority: 9, conditional: s=> HardVore(s) && InPouch(s) == false),
            ...
            priority: 9, conditional: s=> Friendly(s) && ActorHumanoid(s) && InPouch(s) == false),
            ...
            //Kangaroo pred (pouch vore)
            new EventString((i) => $"With no fanfare or ceremony, <b>{i.Unit.Name}</b>'s pouch stops moving, signaling <b>{i.Target.Name}</b>'s death within.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            new EventString((i) => $"Within <b>{i.Unit.Name}</b>'s pouch, <b>{i.Target.Name}</b> finally stops moving, succumbing to {GPPHis(i.Target)} fate.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            new EventString((i) => $"With no new oxygen for far longer than is safe, <b>{i.Target.Name}</b> finally stops moving within <b>{i.Unit.Name}</b>'s pouch.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            ...
            new EventString((i) => $"With no new oxygen for far longer than is safe, <b>{i.Target.Name}</b> finally stops moving within <b>{i.Unit.Name}</b>'s pouch. <b>{i.Target.Name}</b>'s last concious thought is a simple question: \"Why me? What did I do?\"", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            ...
            new EventString((i) => $"With no new oxygen for far longer than is safe, <b>{i.Target.Name}</b> finally stops moving within <b>{i.Unit.Name}</b>'s pouch. <b>{i.Target.Name}</b>'s last concious thought is a simple question: \"When will the battle end?\"", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            ...
            priority: 10, conditional: s => TargetLeader(s) && ActorHumanoid(s) && InPouch(s) == false),
            ...
            targetRace: Race.RwuMercenaries, priority: 9, conditional: (s) => InPouch(s) == false),
            //FeralEevee Digests
            new EventString((i) => $"<b>{PotentialNextPrey(i.Unit).Name}</b> approaches <b>{i.Unit.Name}</b> menacingly. Then the Eevee looks at <b>{PotentialNextPrey(i.Unit).Name}</b>, who freezes seeing how cute the Eevee is, and remains frozen until a glorp sound comes from <b>{i.Unit.Name}</b>, followed by a short burp. <b>{i.Target.Name}</b> is dead.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && PotentialNextPrey(s.Unit).Name != "You, the player" && InStomach(s)),
            new EventString((i) => $"As gurgling sound eminates from <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> gut, signaling <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> death, the Eevee wonders where that {GetRaceDescSingl(i.Target)} from earlier went, then gets distracted by a passing cloud and forgets about <b>{i.Target.Name}</b> entirely.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InStomach(s)),
            new EventString((i) => $"As <b>{i.Target.Name}</b> finally sucumbs to <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> stomach, the Eevee(who has already forgotten <b>{i.Target.Name}</b>) looks down and wonders why {GPPHeIsAbbr(i.Unit)} so {GetRandomStringFrom("big", "full", "big and full")}.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InStomach(s)),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> plays, a final jump kills off <b>{i.Target.Name}</b>, the Eevee's womb begining the process of absorbing {GPPHim(i.Target)}.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && Config.KuroTenkoEnabled == false && InWomb(s)),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> plays, a final jump knocks <b>{i.Target.Name}</b> out, allowing the Eevee's womb to get down to the buisness of turning the {GetRaceDescSingl(i.Target)} into an Eevee.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && Config.KuroTenkoEnabled && (Config.UBConversion == UBConversion.Both || Config.UBConversion == UBConversion.RebirthOnly) && InWomb(s)),
            new EventString((i) => $"A pleasurable feeling rushes through <b>{i.Unit.Name}</b>, signaling the death of <b>{i.Target.Name}</b> within {GPPHis(i.Unit)} womb. Of course, the small pokemon doesn't realize any of this, and instead runs around looking for someone to play with.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => TargetHumanoid(s) && InWomb(s)),
        ...
        AbsorptionMessages = new List<EventString>()
        {
            ...
            priority: 8, conditional: s => WeightGain(s) && InPouch(s) == false),
            ...
            priority: 8, conditional: s=> WeightGain(s) && InPouch(s) == false && (CanAddressPlayer(s) || PotentialNextPrey(s.Unit).Name != "You, the player" )),
            ...
            priority: 9, conditional: s => ActorHumanoid(s) && Condoms(s)),
            //More Condom Disposal credits to Tatltuae!
            ...
            //Kangaroo pred (pouch vore)
            new EventString((i) => $"As <b>{i.Unit.Name}</b>'s {GetRandomStringFrom("lower torso", "pouch", "marsupium")} shrinks back to its original shape, <b>{i.Unit.Name}</b>'s pouchs entrance unseals, revealing nothing inside, as though <b>{i.Target.Name}</b> never existed.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> slowly pulls open {GPPHis(i.Unit)} pouch, finding nothing inside at all.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b>'s pouch unseals, ready to hold more{GetRandomStringFrom(" occupants", " (temporary) occupants", " \"occupants\"")}.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b>'s pouch has shrunk back down to normal size and shape, with no evidence that a {GetRaceDescSingl(i.Target)} had ever been inside.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            new EventString((i) => $"With <b>{i.Target.Name}</b> fully absorbed, <b>{i.Unit.Name}</b>'s pouch unseals, revealing not one hint as to what happened to its previous {GetRandomStringFrom("occupant", "tenant")}.", actorRace: Race.Kangaroos, priority: 10, conditional: s=> InPouch(s)),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> eagerly soaks up eveything <b>{i.Target.Name}</b> has to offer, needing every drop of meat she could digest to grow big and strong!",
            actorRace: Race.Feit, priority: 11),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> exhales deeply, pushing out a long series of {PreyLocStrings.ScatAdjSyn()} turds from her ass end as she piles up <b>{i.Target.Name}'s</b> remnants as {PreyLocStrings.ScatSyn()}.",
            ...
            actorRace: Race.FeralOrcas, targetRace: Race.FeralSharks, priority: 9, conditional: s => Scat(s)),
            ...
            //FeralEevee Absorbs
            new EventString((i) => $"<b>{i.Unit.Name}</b> feels a familiar pressure at {GPPHis(i.Unit)} anus and squeezes out <b>{i.Target.Name}</b> as a large pile of Eevee poo. Of course, <b>{i.Unit.Name}</b> doesn't understand that that's what {GPPHeIsAbbr(i.Unit)} doing, and walks away with no memory of <b>{i.Target.Name}</b> at all.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => Scat(s)),
            new EventString((i) => $"As what little remains of <b>{i.Target.Name}</b> slips out of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> anus, the Eevee idly wonders where <b>{i.Target.Name}</b> went, before forgetting the {GetRaceDescSingl(i.Target)} forever.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => Scat(s)),
            new EventString((i) => $"Feeling the need to poop, <b>{i.Unit.Name}</b> blushes and pushes out the remains of <b>{i.Target.Name}</b> as stealthily as {GPPHe(i.Unit)} can, before walking away.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => Scat(s)),
            new EventString((i) => $"With a warm feeling, the liquid remains of <b>{i.Target.Name}</b> pour from <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}, who for {GPPHis(i.Unit)} part looks quite satisfied at the feeling, but confused as to the cause.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s)),
            new EventString((i) => $"As the last viable remains of <b>{i.Target.Name}</b> are drained into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> fallopian tubes, <b>{i.Target.Name}</b> becomes little more than ovums awaiting their chance to become Eevees.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s)),
            new EventString((i) => $"From the outside, it would appear that <b>{i.Target.Name}</b> is entirely gone. From <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> own perspective, {GPPHe(i.Target)} come{SIfSingular(i.Target)} to {GPPHis(i.Target)} senses and realize that {GPPHe(i.Unit)} {HasHave(i.Unit)} become a part of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> womb, and will not be leaving any time soon.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => InWomb(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> is back to normal size, and is wondering where that {GetRaceDescSingl(i.Target)} from earlier went. {Capitalize(GPPHe(i.Target))} {WasWere(i.Unit)} fun!",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s)),
            new EventString((i) => $"As the last of <b>{i.Target.Name}</b> is absorbed into <b>{i.Unit.Name}</b>, the playful creature runs off, with <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> entire life reduced to pudge on the body of a creature that doesn't remember {GPPHim(i.Target)} at all.",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s)),
            new EventString((i) => $"As {GPPHis(i.Unit)} belly shrinks back to normal size, <b>{i.Unit.Name}</b> thinks on just how many playmates have gotten lost in there. Maybe that {GetRaceDescSingl(i.Target)} will help them all find their way out?",
            actorRace: Race.FeralEevee, priority: 11, conditional: s => InStomach(s)),
            new EventString((i) => $"Noticing that {GPPHis(i.Unit)} belly is back to normal size, <b>{i.Unit.Name}</b> figures that <b>{i.Target.Name}</b> must have found some way out. After all, <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> stomach is 100% safe. \n<b>{i.Unit.Name}</b> lets out a small burp. Yeah, definitly 100% safe.",
        ...
        {
            //Additional logs as well as the Youko and Aabayx exclusives by Tatltuae
        ...
        RebirthMessages = new List<EventString>()
        {
            ...
            //Huge credit to Tatltuae for all of the new logs!
            ...
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> time inside <b>{i.Unit.Name}</b> is complete and {GPPHe(i.Target)} {IsAre(i.Unit)} pushed out: \nCongratulations, your <b>{i.Target.Name}</b> evolved into an Eevee!",priority: 11, actorRace: Race.FeralEevee, conditional: s => InWomb(s) && s.RebirthType == 1),
            new EventString((i) => $"As <b>{i.Target.Name}</b> emerges from <b>{i.Unit.Name}</b>, {GPPHe(i.Target)} {IsAre(i.Unit)} momentarily shocked by {GPPHis(i.Target)} transformation into an Eevee, before promptly forgetting {GPPHis(i.Target)} former life.",priority: 11, actorRace: Race.FeralEevee, conditional: s => InWomb(s) && s.RebirthType == 1),
            new EventString((i) => $"As <b>{i.Target.Name}</b> is pushed out of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}, {GPPHe(i.Target)} {IsAre(i.Unit)} overwhelmed by love for {GPPHis(i.Target)} new \"mother,\" and a strange desire to play and have fun.",priority: 11, actorRace: Race.FeralEevee, conditional: s => InWomb(s) && s.RebirthType == 2),
            new EventString((i) => $"As <b>{i.Target.Name}</b> remerges from <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {GetRandomStringFrom("vagina", "muff", "cunt", "pussy")}, {GPPHis(i.Target)} body is unchanged, but {GPPHe(i.Target)} clearly now have the mind of an Eevee.",priority: 11, actorRace: Race.FeralEevee, conditional: s => InWomb(s) && s.RebirthType == 2),
        ...
        {
            new EventString((i) => $"<b>{i.Unit.Name}</b> jiggles their {PreyLocStrings.ToSyn(i.preyLocation)} with <b>{i.Target.Name}</b> stashed inside. \"What are you waiting for in there?\" - {GPPHe(i.Unit)} asks, annoyed.",priority:25, conditional: s => HasGreatEscape(s) && ActorHumanoid(s) && InPouch(s) == false),
            new EventString((i) => $"<b>{i.Unit.Name}</b> shakes their {GetRandomStringFrom("lower torso", "pouch", "marsupium")} with <b>{i.Target.Name}</b> stashed inside. \"What are you waiting for in there?\" - {GPPHe(i.Unit)} asks, annoyed.",priority:25, conditional: s => HasGreatEscape(s) && ActorHumanoid(s) && InPouch(s)),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {PreyLocStrings.ToSyn(i.preyLocation)} is quiet. Too quiet. \"Did you digest in there?\" - <b>{i.Unit.Name}</b> asks prodding {GPPHis(i.Unit)} {PreyLocStrings.ToSyn(i.preyLocation)}. Nope, <b>{i.Target.Name}</b> is still there.",priority:25, conditional: s => HasGreatEscape(s) && ActorHumanoid(s) && InPouch(s) == false),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {GetRandomStringFrom("lower torso", "pouch", "marsupium")} is quiet. Too quiet. \"Did you digest in there?\" - <b>{i.Unit.Name}</b> asks prodding {GPPHis(i.Unit)} {PreyLocStrings.ToSyn(i.preyLocation)}. Nope, <b>{i.Target.Name}</b> is still there.",priority:25, conditional: s => HasGreatEscape(s) && ActorHumanoid(s) && InPouch(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> stretches, feeling the pleasant heaviness of <b>{i.Target.Name}</b> in {GPPHis(i.Unit)} {PreyLocStrings.ToSyn(i.preyLocation)}.",priority:25, conditional: (s) => HasGreatEscape(s) && InPouch(s) == false),
            new EventString((i) => $"<b>{i.Unit.Name}</b> stretches, feeling the pleasant heaviness of <b>{i.Target.Name}</b> in {GPPHis(i.Unit)} {GetRandomStringFrom("lower torso", "pouch", "marsupium")}.",priority:25, conditional: (s) => HasGreatEscape(s) && InPouch(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> kicks the {PreyLocStrings.ToSyn(i.preyLocation)} holding {GPPHim(i.Target)} from the inside. <b>{i.Unit.Name}</b> considers getting rid of {GPPHis(i.Unit)} rude occupant right now.",priority:25, conditional: (s) => HasGreatEscape(s) && InPouch(s) == false),
            new EventString((i) => $"<b>{i.Target.Name}</b> kicks the {GetRandomStringFrom("lower torso", "pouch", "marsupium")} holding {GPPHim(i.Target)} from the inside. <b>{i.Unit.Name}</b> considers getting rid of {GPPHis(i.Unit)} rude occupant right now.",priority:25, conditional: (s) => HasGreatEscape(s) && InPouch(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> curls up within claustrophobic confines of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {PreyLocStrings.ToSyn(i.preyLocation)}.",priority:25, conditional: (s) => HasGreatEscape(s) && InPouch(s) == false),
            new EventString((i) => $"<b>{i.Unit.Name}</b> has a hard time keeping <b>{i.Target.Name}</b> in {GPPHis(i.Unit)} {PreyLocStrings.ToSyn(i.preyLocation)}.",priority:25, conditional: (s) => HasGreatEscape(s) && InPouch(s) == false),
            new EventString((i) => $"<b>{i.Unit.Name}</b> moans, rubbing {GPPHis(i.Unit)} {PreyLocStrings.ToSyn(i.preyLocation)}. <b>{i.Target.Name}</b> proves to be a hardy catch",priority:25, conditional: (s) => HasGreatEscape(s) && InPouch(s) == false),
          ...
          targetRace: Race.Erin, priority: 26, conditional: s => Cursed(s) && Lewd(s) && InPouch(s) == false),
        ...
        {
            new EventString((i) => $"<b>{i.Unit.Name}</b> stretches and goes to pat {GPPHis(i.Unit)} full {PreyLocStrings.ToSyn(i.preyLocation)}, only to notice with horror the emptiness where <b>{i.Target.Name}</b> was supposed to be.",priority:25, conditional: (s) => InPouch(s) == false),
            new EventString((i) => $"<b>{i.Unit.Name}</b> stretches and goes to pat {GPPHis(i.Unit)} full {GetRandomStringFrom("lower torso", "pouch", "marsupium")}, only to notice with horror the emptiness where <b>{i.Target.Name}</b> was supposed to be.",priority:25, conditional: (s) => InPouch(s)),
            ...
            new EventString((i) => $"Tired of not being able to digest <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> decides to let {GPPHim(i.Target)} out.",priority:25),
            new EventString((i) => $"<b>{i.Target.Name}</b> slips out of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {PreyLocStrings.ToSyn(i.preyLocation)}.",priority:25, conditional: (s) => InPouch(s) == false),
