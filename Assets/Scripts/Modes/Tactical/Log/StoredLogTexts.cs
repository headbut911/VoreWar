...
static class StoredLogTexts
{
    ...
    internal static void InitializeLists()
    {
        ...
        AbsorptionMessages = new List<EventString>()
        {
            ...
            //Condom Disposal credits to Cartography!
        ...
        TransferMessages = new List<EventString>()
        {
            ...
            priority: 11, conditional: s => Config.CondomsForCV && !InStomach(s) && Lewd(s) && s.Prey.IsDead ),
            ...
            priority: 11, conditional: s => Config.CondomsForCV && !InStomach(s) && Lewd(s) && s.Prey.IsDead && ReqRAW(s)),
        ...
        VoreStealMessages = new List<EventString>()
        {
            ...
            priority: 8),
            //Unbirth vore steal
            new EventString((i) => $"As <b>{i.Unit.Name}</b> pleasures <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {PreyLocStrings.ToSyn(PreyLocation.womb)}, in a moment of ecstasy {GPPHe(i.Target)} relaxes allowing <b>{i.Prey.Name}</b> to break free. However this freedom is short-lived as <b>{i.Prey.Name}</b> is {GetRandomStringFrom("immediately", "instanlty", "swiftly")} {GetRandomStringFrom("eaten up", "devoured", "swallowed up", "slurped up", "wolfed down")} by <b>{i.Unit.Name}</b>.",
            priority: 10, conditional: s => s.oldLocation == PreyLocation.womb && s.preyLocation == PreyLocation.stomach && Lewd(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> pries apart <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> vagina {GetRandomStringFrom("freeing", "liberating", "springing")} <b>{i.Prey.Name}</b>, only to {GetRandomStringFrom("immediately", "instanlty", "swiftly")} send the freed {GetRaceDescSingl(i.Prey)} into {GPPHis(i.Unit)} own {PreyLocStrings.ToSyn(PreyLocation.stomach)}.",
            priority: 8, conditional: s => s.oldLocation == PreyLocation.womb && s.preyLocation == PreyLocation.stomach),
            //Tail vore steal
            ...
            priority: 8, conditional: s => s.oldLocation == PreyLocation.tail && s.preyLocation == PreyLocation.stomach),
        };
