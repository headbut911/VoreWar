...
static class StoredLogTexts
{
    ...
    internal static void InitializeLists()
    {
        ...
        SwallowMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"After {GetRandomStringFrom("swallowing", "gulping down", "voring", "devouring")} <b>{i.Target.Name}</b>, <b>{i.Unit.Name}</b> taunts \"Ta-da! No hands!\" However, <b>{i.Target.Name}</b> {GetRandomStringFrom("is not amused by the trick.", "is not happy one bit.", "is unimpressed.", $"remains displeased at {GPPHis(i.Target)} current situation.")}",
            actorRace: Race.Xelhilde, priority: 9, conditional: s => TargetHumanoid(s) && !s.Target.IsDead),
            new EventString((i) => $"<b>{i.Unit.Name}</b> {GetRandomStringFrom("swallows", "gulps down", "vores", "devours")} <b>{i.Target.Name}</b> {GetRandomStringFrom("tauntingly", "mockingly", "indignantly")} and {GetRandomStringFrom("barks", "says")} \"Funny how I can't use my paws to wield my weapon yet you can, and you <i>still</i> lost! Well, I hope you enjoy your stay in there because I know I will!\" <b>{i.Target.Name}</b> now begins to stew in TWO ways after such a {GetRandomStringFrom("scathing", "harsh", "brutal", "salty")} remark from a semi-feral {GetRandomStringFrom("hound", "canine", "husky", "dog")}.",
